...
def clean_description(description_str: str) -> str | None:
    ...
    description_without_html = re.sub(r'<[^>]*>', ' ', description_without_html)
...
def process_bronze_to_gold():
    ...
    with SessionLocal() as db:
        ...
        for doc in raw_documents:
            ...
            if not url:
                logging.warning(f"Skipping document {doc['_id']} due to missing URL.")
                continue
            ...
            if not all([title_cleaned, price_cleaned, url_cleaned]):
                logging.warning(f"Skipping URL {url_cleaned} due to incomplete data after cleaning.")
                continue

            # Map refined data to the PostgreSQL relational model    
            new_product = Product(title=title_cleaned, price=price_cleaned, url=url_cleaned, rating=rating_cleaned, description=description_cleaned)
            ...
            try:
                ...
                db.commit()
                processed_count += 1
                logging.info(f"Product refined and saved: {title_cleaned} | Price: {price_cleaned}")
            except Exception as e:
                # Rollback transaction on failure to maintain database integrity
                db.rollback()
                logging.error(f"Failed to save product from URL {url}: {e}")
            
    logging.info(f"Refinement process completed. New products: {processed_count}, Skipped (existing): {skipped_count}.")
